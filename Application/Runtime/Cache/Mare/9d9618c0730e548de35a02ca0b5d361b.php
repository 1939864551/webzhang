						...
						<div class="col-lg-6">
							<div class="block-content">
								...
								<div class="block-body">
							                  	...
							                  	<div class="input-group input-group-file">
							                  		<span class="input-group-btn">
							                        	...
							                        	<input type="file" name="uploadfile" multiple>
							                      		...
							                      		<input type="hidden"  name="addtime" id="addtime" value="<?php echo (session('addtime')); ?>"/>
							                    	</span>
									...
									<?php if(C("ISDYNAMIC_MODE")== 1): ?><div class="form-group row">
											<div class="col-md-3">
						...
						<div class="col-lg-6" id="high-test" style="margin-bottom: 50px;">
							<div class="block-content">
								...
								<div class="block-body" id='checkboxchoice'>
									<?php if(is_array($ruletypelist)): $i = 0; $__LIST__ = $ruletypelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vru1): $mod = ($i % 2 );++$i; if($vru1['key'] != 10): ?><div <?php if($vru1['available'] == 'apk'){echo "class='apk form-group'";}else{ echo 'class="form-group"'; } ?>>
										<p><?php echo ($vru1['value']); ?></p>
										<div class="clearfix row-bottom">
											<?php if(is_array($vru1['rulelist'])): $i = 0; $__LIST__ = $vru1['rulelist'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vru2): $mod = ($i % 2 );++$i;?><div class="form-group">
												<label class="c-input c-input-checkbox" for="gaofengxian<?php echo ($vru2['id']); ?>">
													<input class="c-checkbox" type="checkbox" name="userule[]" id="gaofengxian<?php echo ($vru2['id']); ?>" value="<?php echo ($vru2['id']); ?>">
													<span class="c-icon-checked"></span>
													<span><?php echo ($vru2['rulesname']); ?><span>
													<span style="margin-left: 10px;"><?php if( strlen($vru2['rulesinfo']) >20 ){ echo substr($vru2['rulesinfo'],0,18).'...'; }else{ echo $vru2['rulesinfo']; } ?></span>
												</label>
...
<div class="block-body">
	...
	var uploadSuccessTips = '<div id="action-tips" class="alert fade in alert-success global-tips" role="alert">上传成功</div>';
    	var is_dynameics = "<?php echo (C("ISDYNAMIC_MODE")); ?>";
		var url = "<?php echo U('Mare/Task/task_upload/testtype/app');?>";
		...
		$("#addtaskform").validate({
			...
			submitHandler:function(form){
				...
				var url = "<?php echo U('Mare/Task/task_add');?>";
	            $.ajax({
	            	type:"post",
					url:url,
                    timeout:15000,
                    ...
                    complete : function(XMLHttpRequest,status){
                        if(status=='timeout'){
                            ...
                            // setTimeout(function(){
                            //     $("#action-tips").alert("close");
                            //     window.location.href = "<?php echo U('Mare/Task/task_select_type');?>";
                            // },3000)
                        }
                    }
